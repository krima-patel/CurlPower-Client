    ...
    <>
      ...
      <div className="d-flex flex-wrap">
        {products?.map((product) => (
          <ProductCard key={`product--${product.id}`} id={product.id} imageUrl={product.image_url} productType={product.product_type} name={product.name} purpose={product.purpose} date={product.date} onUpdate={getAllTheProducts} />
        ))}
      </div>
    </>
  );
